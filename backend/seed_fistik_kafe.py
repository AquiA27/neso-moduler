...
# ===== KONFİGÜRASYON =====
DEMO_ISLETME_ADI = "Fıstık Kafe"
DEMO_SUBE_ADI = "Merkez Şube"
DEMO_ADMIN_USER = "fistik_admin"
DEMO_ADMIN_PASS = "FistikAdmin123!"  # Güçlü şifre (production policy)
DEMO_OPERATOR_USER = "fistik_kasiyer"
DEMO_OPERATOR_PASS = "Kasiyer123!"
DEMO_MUTFAK_USER = "fistik_mutfak"
DEMO_MUTFAK_PASS = "Mutfak1234!"
DEMO_GARSON_USER = "fistik_garson"
...
async def main():
    ...
    conn_str = os.getenv(
        "DATABASE_URL",
        "postgresql://postgres:rELURsUWOMKoXFJMgJrqQzuLcGajERty@maglev.proxy.rlwy.net:46270/railway"
    )
    ...
    try:
        ...
        # Son 60 gün (2 ay) için sipariş oluştur
        ...
        for day_offset in range(60, 0, -1):
            ...
            # Aylık giderler (Ayın 1'inde)
            if tarih.day == 1:
                kisiler_vb = [
                    ("Kira", "Aylık Kira Ödemesi", 25000),
                    ("Personel", "Maaşlar (Aylık)", 65000),
                    ("Elektrik", "Aylık Elektrik Faturası", 4500),
                    ("Su", "Aylık Su Faturası", 1200),
                    ("İnternet", "Aylık İnternet Faturası", 450)
                ]
                for kat, aciklama, tutar in kisiler_vb:
                    await conn.execute(
                        "INSERT INTO giderler (sube_id, kategori, aciklama, tutar, tarih) VALUES ($1, $2, $3, $4, $5)",
                        sube_id, kat, f"{tarih.strftime('%B %Y')} - {aciklama}", tutar, tarih
                    )
                    gider_count += 1
            
            # Haftalık giderler
            if day_offset % 7 == 0:
                await conn.execute("INSERT INTO giderler (sube_id, kategori, aciklama, tutar, tarih) VALUES ($1, $2, $3, $4, $5)",
                    sube_id, "Hammadde", "Haftalık Sebze & Meyve Alımı", random.randint(3000, 5000), tarih)
                await conn.execute("INSERT INTO giderler (sube_id, kategori, aciklama, tutar, tarih) VALUES ($1, $2, $3, $4, $5)",
                    sube_id, "Hammadde", "Haftalık Kasap Alımı", random.randint(6000, 9000), tarih)
                await conn.execute("INSERT INTO giderler (sube_id, kategori, aciklama, tutar, tarih) VALUES ($1, $2, $3, $4, $5)",
                    sube_id, "Hammadde", "Haftalık Kuru Gıda Alımı", random.randint(4000, 7000), tarih)
                await conn.execute("INSERT INTO giderler (sube_id, kategori, aciklama, tutar, tarih) VALUES ($1, $2, $3, $4, $5)",
                    sube_id, "Temizlik", "Haftalık Temizlik Malzemesi", random.randint(750, 1500), tarih)
                gider_count += 4
                
            # Rastgele diğer giderler (%15 ihtimal)
            if random.random() < 0.15:
                await conn.execute("INSERT INTO giderler (sube_id, kategori, aciklama, tutar, tarih) VALUES ($1, $2, $3, $4, $5)",
                    sube_id, "Pazarlama", "Instagram Sponsorlu Reklam Kampanyası", 2500, tarih)
                gider_count += 1
            if random.random() < 0.10:
                await conn.execute("INSERT INTO giderler (sube_id, kategori, aciklama, tutar, tarih) VALUES ($1, $2, $3, $4, $5)",
                    sube_id, "Bakım", "Makine Bakım Onarım", 3000, tarih)
                gider_count += 1

        print(f"   ✅ {gider_count} gider kaydı eklendi")
        ...
        print(f"   Siparişler: {siparis_count} (son 60 gün)")
        print(f"   Ödemeler : {odeme_count} kayıt")
        print(f"   Giderler : {gider_count} kayıt")
        print()
        print("🔑 TANITIM İÇİN:")
